...
router.get('/qryreqlist', function (req, res, next) {
    ...
    req.session.touch();
    // 查明细列表
    let qryString = "select a.req_id,b.merchant_name,date_format(a.req_time,'%Y-%m-%d %H:%i:%s') as req_time,a.raw_msg, a.rs_status from mo_command a left join merchant b on a.merchant_id=b.merchant_id where 1=1";
    let qryParams = [];
    ...
    // 查总数
    qryString = "select count(1) as total from mo_command a left join merchant b on a.merchant_id=b.merchant_id where 1=1";
    if(req.query.paraName && req.query.paraName != ""){
        qryString += " and b.merchant_name like ? "
        qryParams.push("%"+req.query.paraName+"%");
    }
